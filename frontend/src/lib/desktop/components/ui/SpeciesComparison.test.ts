...
describe('SpeciesComparison', () => {
  ...
  it('shows loading indicator when fetching guide for selected species', async () => {
    ...
    global.fetch = vi.fn((url: string) => {
      ...
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve({ description: 'Default' }),
      });
    }) as unknown as typeof fetch;
    ...
    // Wait for similar species to load and auto-selection to trigger guide fetch
    await waitFor(() => {
      expect(screen.getAllByText('Song Thrush').length).toBeGreaterThan(0);
    });

    // The first species should be auto-selected and show loading indicator
