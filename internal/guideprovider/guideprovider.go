	...
	if cached, ok := c.dataMap.Load(scientificName); ok {
		...
		if guide.IsNegativeEntry() {
			if !isCacheEntryStale(guide.CachedAt, true) {
				// Negative cache hit: species is known to have no guide.
				// Record as CacheHit (we found the entry) with not_found quality.
				if c.metrics != nil {
			...
			// Return a copy so callers cannot mutate the cached struct's slice fields.
			guideCopy := *guide
