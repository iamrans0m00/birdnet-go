...
func (c *GuideCache) Get(ctx context.Context, scientificName string, opts FetchOptions) (*SpeciesGuide, error) {
	...
	// Tier 2: DB cache
	guide, err = c.checkDBCache(ctx, scientificName, providerName)
	if !errors.Is(err, errCacheMiss) {
		return guide, err
	}
...
func (c *GuideCache) checkMemoryCache(scientificName string, opts FetchOptions, providerName string) (*SpeciesGuide, error) {
	cached, ok := c.dataMap.Load(scientificName)
	if !ok {
...
// Returns ErrGuideNotFound for a fresh negative-cache hit.
// Returns nil error with a non-nil guide on a positive hit.
func (c *GuideCache) checkDBCache(ctx context.Context, scientificName, providerName string) (*SpeciesGuide, error) {
	if c.store == nil {
		return nil, errCacheMiss
	}
...
func (c *GuideCache) refreshStaleEntries() {
	...
	// Update cache hit ratio metric — reuse the already-fetched entries slice.
	c.updateCacheHitRatio(entries)
}
...
// updateCacheHitRatio calculates and updates the cache hit ratio metric from
// a pre-fetched slice, avoiding a redundant database query.
func (c *GuideCache) updateCacheHitRatio(entries []GuideCacheEntry) {
	if c.metrics == nil {
		return
	}
